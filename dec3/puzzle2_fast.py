...
def plot(directions):
    ...
    for p in directions:
        ...
        num = int(num)
        if d == "R":
            lines.append((curr, (curr[0]+num, curr[1]), True, True, dist))
            curr = (curr[0]+num, curr[1])
        elif d == "L":
            lines.append(((curr[0]-num, curr[1]), curr, True, False, dist))
            curr = (curr[0]-num, curr[1])
        elif d == "U":
            lines.append(((curr[0], curr[1]-num), curr, False, False, dist))
            curr = (curr[0], curr[1]-num)
        elif d == "D":
            lines.append((curr, (curr[0], curr[1]+num), False, True, dist))
            curr = (curr[0], curr[1]+num)
        dist += num
    return lines
        ...
        if set([line1[2], line2[2]]) == {True, False}:
            if line1[2] == True:
                if line1[0][0] < line2[0][0] < line1[1][0] and line2[0][1] < line1[0][1] < line2[1][1]:
                    ...
                    if line2[3] else line2[1][1]-line1[0][1])
                    if currsum < minsum:
                        minsum = currsum
            else:
